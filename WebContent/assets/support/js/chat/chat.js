...
doc.ready(() => {
	...
	new SocketHelper(currentUser.id, (result) => {
		switch (result.event) {
			case 'sendMessage':
				...
				roomToFirst(result)
				break
			case 'edit_nick_name': case 'edit_group_name':
	...
	const searchEvent = () => {
		ajaxHelper(`${Modes.SEARCH}/${searchInput.val()}`, null, () => {
		}, (result) => {
			result.map(user => {
	...
	const actionMenuContentClick = {
		...
		'create_group': () => {
			ajaxHelper(`${Modes.CREATE_GROUP}`, null, () => {
			}
				, (result) => {
					window.location.replace('chat/' + result.id)
				}, (XMLHttpRequest, textStatus, errorThrown) => {
		...
		'add_group': () => {
			...
			ajaxHelper(tailUrl, null, () => {
				...
				results.rooms.map((room) => {
					if (results.rooms[0].id == undefined) {
						return
					}
					$('.roomsjoined').find('.rooms').append(roomsJoined(room))
				})
		...
		'leave_group': () => {
			ajaxHelper(`${Modes.LEAVE_GROUP}/${idCurrentRoom}`, null, () => {
			}
				, (result) => {
	...
	const inputHeplerEvent = (url, placeholder) => {
		...
		$('.submit_action_menu').click(() => {
			ajaxHelper(url, $('.input_helper').val().trim(), () => {
			}, (result) => {
				hideHelper()
			}, (XMLHttpRequest, textStatus, errorThrown) => {
			})
	...
	sendBtn.click((event) => {
		...
		if (idCurrentRoom != undefined) {
			...
			scrollBottom()
			ajaxHelper(`${Modes.MESSAGE}/${idCurrentRoom == null ? idPartner : idCurrentRoom}`, message, () => {
			}
				, (result) => {
				}, (XMLHttpRequest, textStatus, errorThrown) => {
					console.log(errorThrown)
